# Graph Report - chatting-management-tool  (2026-07-07)

## Corpus Check
- 167 files · ~136,417 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1095 nodes · 1744 edges · 70 communities (64 shown, 6 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 94 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c10131db`
...
Cohesion: 0.10
Nodes (24): buildGroups(), chipStyle(), copy(), dayHeader, filterLabel, filterPanel, filterRow, fmtActioned() (+16 more)

### Community 2 - "Creator Stats Parsing"
Cohesion: 0.15
Nodes (15): DismissModal(), ghost, primary, AIQualityPanel(), inp, SettingsPage(), getInflowwOffset(), setInflowwOffset() (+7 more)
...
Cohesion: 0.11
Nodes (10): Avatar(), DAY_LABELS, DAY_NUMBERS, getTodayDayNumber(), getWeekDates(), ShiftSlot(), WeekBar(), avatarColor() (+2 more)
...
Cohesion: 0.11
Nodes (30): prioritiseTasks(), { runAgentDetailed }, shiftDays(), { supabaseAdmin }, { buildTasksForDate, capLiveQueue, buildSpenderDevelopmentTasks }, crypto, { prioritiseTasks }, { requireMinRole } (+22 more)
...
Cohesion: 0.07
Nodes (35): days(), dollar(), extractDate(), { findOrCreateCreator }, intval(), nullDash(), parseCreatorStats(), pct() (+27 more)
...
Cohesion: 0.06
Nodes (44): extractDate(), { findOrCreateCreator }, nullDash(), parseCreatorStats(), parseDays(), parseDollar(), parseFloat2(), parsePercent() (+36 more)
...
Cohesion: 0.17
Nodes (15): { buildOverview }, { evaluateChatterDay }, { evaluateChatterSales }, { evaluateCreatorDay }, NOTE: must be registered before '/:date' or it gets caught as a date param., router, { runDailyCheck }, { saveEvaluation, getEvaluations, getEvaluationsForDate, getLatestEvaluations } (+7 more)
...
### Community 51 - "computeChatterMetrics.js"
Cohesion: 0.39
...
- **Why does `supabaseAdmin` connect `shifts.js` to `Report Parsers`, `Daily Check Engine`, `ai.js`, `creators.js`, `Daily Check Page UI`, `Custom Task Creation`, `Tasks Page UI`, `Server Dependencies`, `Creators Page UI`, `ESLint Config`, `Chatter Profile UI`, `App Shell & Routing`, `computeChatterMetrics.js`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `supabaseAdmin` connect `Auth Context & API Client` to `Metrics Computation`, `tasks.js`, `Theme & Topbar`, `Vite Config`, `Review Task Generation`?**
