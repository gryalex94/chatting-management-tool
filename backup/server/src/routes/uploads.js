...
router.post('/message-dashboard', requireMinRole('manager'), upload.single('file'), async (req, res) => {
  try {
    ...
    try {
      ...
      // Auto-populate chatter metrics from employee stats
      const { populateMetricsFromEmployeeStats } = require('../utils/computeMetrics');
      populateMetricsFromEmployeeStats(req.user.organisationId).catch(err =>
...
router.post('/employee-report', requireMinRole('manager'), upload.single('file'), async (req, res) => {
  try {
    ...
    try {
      ...
      // Auto-compute metrics from messages (P50, P90, AFK)
      const { computeMetricsForOrg } = require('../utils/computeMetrics');
      computeMetricsForOrg(req.user.organisationId).catch(err =>
